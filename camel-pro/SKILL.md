...
4. **Scope verification to what changed, not the whole project — and defer confirmation-only runs to camel-dam.** When you do check something yourself, check the specific test or function affected, not the full suite; diff the relevant hunk, not the whole history. Don't run a full suite/build/dev-server pass "at the end to be sure" — `camel-dam`'s rule already covers that: name the command and the expected result, let the user run it. The exceptions in `camel-dam` still apply here (a test you're actively iterating on, a check with no simple user-facing equivalent, an explicit ask, or an unattended run).
...
- **Honest coverage.** If a review/test/check was cut short, rule 8 applies — say so, don't report full confidence you don't have.
